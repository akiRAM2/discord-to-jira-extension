...
- **作成前の確認・編集**: Jiraに送る課題タイトルと本文を作成前に確認し、本文はその場で編集できます。
...
3. **作成画面で確認**
   画面上にチケット作成ウィンドウが出ます。
   - **接頭辞/エピック**: `[Discord]` `[Bug]` などを選ぶと、紐付いた親課題が自動で設定されます。
   - **タイトル**: 自動で入っていますが、わかりやすく書き換えることもできます。
   - **確認・編集**: 作成される課題タイトルを確認し、本文を編集できます。
...
- **タイトルテンプレート**:
    - Jiraチケットのタイトル初期値を自由にカスタマイズできます。
    - デフォルトは `{message} ({author}) in #{channel} ,{server}` です。
    - `{author}`, `{server}`, `{channel}`, `{time}`, `{link}`, `{content}`, `{selection}`, `{message}` も使えます。
- **説明文テンプレート**:
